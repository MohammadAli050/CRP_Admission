...
namespace Admission.Admission.Office
{
    public partial class DistrictSeatLimitSetup : PageBase
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (districtId > 0 && admissionUnitId > 0 && acaCalId > 0 && (!string.IsNullOrEmpty(seatLimit) && Convert.ToInt32(seatLimit) > 0))
            {
                ...
                try
                {
                    if (obj.ID > 0) //update
                    {
                        ...
                        DAL.DistrictSeatLimit existModel = null;
                        using (var db = new OfficeDataManager())
                        {
                            existModel = db.AdmissionDB.DistrictSeatLimits.Where(x => x.ID != DistrictSeatLimitSetupID
                                                                                && x.AdmissionUnitId == admissionUnitId
                        ...
                        if (existModel != null)
                        {
                            MessageView("Same Session, Faculty and District is already exist !!", "fail");
                            return;
                        }
                        #endregion
                        ...
                        using (var db = new GeneralDataManager())
                        {
                            DAL.DistrictSeatLimit dsl = db.AdmissionDB.DistrictSeatLimits.Where(x => x.ID == DistrictSeatLimitSetupID).FirstOrDefault();
                            if (dsl != null)
                            {
                                dsl.IsActive = cbIsActive.Checked;
                                dsl.SeatLimit = Convert.ToInt32(seatLimit);
                                dsl.ModifiedDate = DateTime.Now;
                                dsl.ModifiedBy = (int)uId;

                                db.Update<DAL.DistrictSeatLimit>(dsl);
                                ...
                                try
                                {
                                    ...
                                    thDLog.NewData = "Faculty:" + ddlAdmissionUnit.SelectedItem.Text + "_" +
                                                     "Session:" + ddlSession.SelectedItem.Text + "_" +
                            ...
                            {
                                MessageView("No data found for update!", "fail");
                                return;
                            }
                    ...
                    else //create new
                    {
                        ...
                        using (var db = new OfficeDataManager())
                        {
                            existModel = db.AdmissionDB.DistrictSeatLimits.Where(x => x.AdmissionUnitId == admissionUnitId 
                                                                                && x.DistrictId == districtId 
                                                                                && x.AcaCalId == acaCalId
                                                                                && x.IsActive == true).FirstOrDefault();
                        }
                        ...
                        {
                            MessageView("Failed to insert data!", "fail");
                            return;
                        }
                    }
                }
