...
{
    public partial class ReceiveFormCertificate : PageBase
    {
        ...
        private void LoadDDL()
        {
            ...
            {
                DDLHelper.Bind<DAL.CertificateAdmissionUnit>(ddlUnitProgram, db.AdmissionDB.CertificateAdmissionUnits.ToList(), "UnitName", "ID", EnumCollection.ListItemType.SelectAll);
                DDLHelper.Bind<DAL.CertificateEducationCategory>(ddlEducationCategory, db.AdmissionDB.CertificateEducationCategories.Where(a => a.IsActive == true).ToList(), "CategoryName", "ID", EnumCollection.ListItemType.SelectAll);
        ...
        private void LoadListView()
        {
            ...
            {
                List<FormRequestListViewObject> list = (from candPayment in db.AdmissionDB.CertificateCandidatePayments
                                                        join candFormSl in db.AdmissionDB.CertificateCandidateFormSls on candPayment.ID equals candFormSl.CandidatePaymentID
                                                        join candidate in db.AdmissionDB.CertificateBasicInfoes on candFormSl.CandidateID equals candidate.ID
                                                        join admSetup in db.AdmissionDB.CertificateAdmissionSetups on candFormSl.AdmissionSetupID equals admSetup.ID
                                                        join admUnit in db.AdmissionDB.CertificateAdmissionUnits on admSetup.AdmissionUnitID equals admUnit.ID
                                                        ...
                                                        }).OrderByDescending(x => x.CandidatePaymentID).Take(200).ToList(); //Take(50) where candPayment.IsPaid == true
                if (list.Count() > 0)
                {
        ...
        private void LoadListView(string searchText, int? acaCalId)
        {
            if (searchText != null)//search using search text
            {
                using (var db = new CandidateDataManager())
                {
                    List<DAL.SPCertificateFormRequestGetBySearchText_Result> list = db.AdmissionDB.SPCertificateFormRequestGetBySearchText(searchText, true).ToList();
                    if (list.Any())
                    {
