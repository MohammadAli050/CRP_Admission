...
{
    public partial class RPTCampusWiseSeatPlan : PageBase
    {
            ...
            if (admUnitId > 0 && acaCalId > 0)
            {
                ...
                try
                {
                    ...
                    if (campusId > 0)
                    {
                        ...
                        using (var db = new OfficeDataManager())
                        {
                            campus = db.AdmissionDB.Campus
                                .Where(c => c.ID == campusId && c.IsActive == true)
                                .FirstOrDefault();
                        }

                        if (campus != null)
                        {
                            centreName = campus.CampusName + ", " + campus.AddressLine;
                        }
                ...
                if (list != null && admSetup != null)
                {
                    if (list.Count > 0)
                    {
                        ...
                        param1.Add(new ReportParameter("CentreName", centreName)); //ddlCampus.SelectedItem.Text
