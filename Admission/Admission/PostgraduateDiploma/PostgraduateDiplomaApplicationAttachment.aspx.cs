...
{
    public partial class PostgraduateDiplomaApplicationAttachment : PageBase
    {
        ...
        private void LoadCandidateData(long cId)
        {
            if (cId > 0)
            {
                ...
                {
                    List<DAL.PostgraduateDiplomaDocument> documentList = db.AdmissionDB.PostgraduateDiplomaDocuments.Where(x => x.CandidateID == cId).ToList();//db.GetAllDocumentByCandidateID_AD(cId);
                    if (documentList != null)
                    {
                        DAL.PostgraduateDiplomaDocument photoDocDetailObjTemp = documentList.Where(c => c.DocumentTypeID == 2).FirstOrDefault();
                        DAL.PostgraduateDiplomaDocument signDocDetailObjTemp = documentList.Where(c => c.DocumentTypeID == 3).FirstOrDefault();

                        DAL.PostgraduateDiplomaDocumentDetail photoDocDetailObj = new DAL.PostgraduateDiplomaDocumentDetail();
                        DAL.PostgraduateDiplomaDocumentDetail signDocDetailObj = new DAL.PostgraduateDiplomaDocumentDetail();

                        if (photoDocDetailObjTemp != null)
                        {
                            photoDocDetailObj = db.AdmissionDB.PostgraduateDiplomaDocumentDetails.Where(x => x.ID == photoDocDetailObjTemp.DocumentDetailsID).FirstOrDefault();
                        }

                        if (signDocDetailObjTemp != null)
                        {
                            signDocDetailObj = db.AdmissionDB.PostgraduateDiplomaDocumentDetails.Where(x => x.ID == signDocDetailObjTemp.DocumentDetailsID).FirstOrDefault();
                        }
        ...
        protected void btnUploadPhoto_Click(object sender, EventArgs e)
        {
            ...
            if (FileUploadPhoto.HasFile)
            {
                ...
                string filePath = "~/Upload/PostgraduateDiplomaCandidate/Photo/";
                string tempFileName = "Temp-CC-Ph-" + cId + fileExtension; // CC for certificate candidate, Ph for Photo
                string tempFilePath = "~/Upload/PostgraduateDiplomaCandidate/TEMP/Photo/";
                ...
                if (contentlength < 204800)
                {
                    try
                    {
                        ...
                        if (File.Exists(Server.MapPath(filePath + fileName)))
                        {
                            ...
                            DAL.PostgraduateDiplomaDocument documentObj = null;
                            using (var dbDocument = new CandidateDataManager())
                            {
                                documentObj = dbDocument.AdmissionDB.PostgraduateDiplomaDocuments
                            ...
                            {
                                DAL.PostgraduateDiplomaDocumentDetail documentDetailObj = null;
                                using (var dbDocumentDetails = new CandidateDataManager())
                                {
                                    documentDetailObj = dbDocumentDetails.AdmissionDB.PostgraduateDiplomaDocumentDetails.Find(documentObj.DocumentDetailsID); //dbDocumentDetails.GetDocumentDetailByID_ND(documentObj.DocumentDetailsID);
                                }
                                if (documentDetailObj != null) //document detail exists, update document details
                                {
                                    ...
                                    {
                                        dbUpdateDocumentDetail.Update<DAL.PostgraduateDiplomaDocumentDetail>(documentDetailObj);
                                    }
                                }
                                else //document detail does not exist, insert new document detail, then update document
                                {
                                    DAL.PostgraduateDiplomaDocumentDetail newDocumentDetailObj = new DAL.PostgraduateDiplomaDocumentDetail();
                                    ...
                                    {
                                        dbInsertDocumentDetail.Insert<DAL.PostgraduateDiplomaDocumentDetail>(newDocumentDetailObj);
                                        newDocumentDetailID = newDocumentDetailObj.ID;
                                    }
                                    if (newDocumentDetailID > 0)
                                    {
                                        DAL.PostgraduateDiplomaDocument documentObjToBeUpdated = null;
                                        using (var dbGetDocumentObj = new CandidateDataManager())
                                        {
                                            documentObjToBeUpdated = dbGetDocumentObj.AdmissionDB.PostgraduateDiplomaDocuments
                                                                    .Where(c => c.CandidateID == cId && c.DocumentTypeID == 2)
                                        ...
                                        if (documentObjToBeUpdated != null)
                                        {
                                            ...
                                            {
                                                dbUpdateDocumentObj.Update<DAL.PostgraduateDiplomaDocument>(documentObjToBeUpdated);
                                            }
                                        }
                                    }
                                }//end if-else
                            ...
                            else //insert document  (first document details, then document)
                            {
                                ...
                                if (newDocumentDetailID > 0)
                                {
                                    DAL.PostgraduateDiplomaDocument newDocumentObj = new DAL.PostgraduateDiplomaDocument();
                                    ...
                                    {
                                        dbInsertDocument.Insert<DAL.PostgraduateDiplomaDocument>(newDocumentObj);
                                    }
                                }
                            }//end if-else
        ...
        protected void btnUploadSignature_Click(object sender, EventArgs e)
        {
            ...
            if (FileUploadSignature.HasFile)
            {
                ...
                string filePath = "~/Upload/PostgraduateDiplomaCandidate/Signature/";
                string tempFileName = "Temp-CC-Sn-" + cId + fileExtension; // CC for certificate candidate, Sn for Signature
                string tempFilePath = "~/Upload/PostgraduateDiplomaCandidate/TEMP/Signature/";
