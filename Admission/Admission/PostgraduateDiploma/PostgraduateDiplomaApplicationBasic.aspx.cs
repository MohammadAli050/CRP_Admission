...
{
    public partial class PostgraduateDiplomaApplicationBasic : PageBase
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            DAL.PostgraduateDiplomaBasicInfo postgraduateDiplomaBasicInfo = new DAL.PostgraduateDiplomaBasicInfo();
            using (var db = new CandidateDataManager())
            {
                postgraduateDiplomaBasicInfo = db.AdmissionDB.PostgraduateDiplomaBasicInfoes.Where(x => x.CandidateUserID == uId).FirstOrDefault();
            }

            if (postgraduateDiplomaBasicInfo == null) //(string.IsNullOrEmpty(Request.QueryString["val"]))
            {
                Response.Redirect("~/Admission/Message.aspx?message=Illegal Page Request&type=warning", false);
            }
            else
            {
                cId = postgraduateDiplomaBasicInfo.ID;//Int64.Parse(Request.QueryString["val"].ToString());
        ...
        private void LoadCandidateData(long cId)
        {
            if (cId > 0)
            {
                ...
                {
                    //basic info-------------------------------------------------------
                    DAL.PostgraduateDiplomaBasicInfo candidate = db.AdmissionDB.PostgraduateDiplomaBasicInfoes.Where(x => x.ID == cId).FirstOrDefault();
        ...
        protected void btnSave_Basic_Click(object sender, EventArgs e)
        {
            ...
            {
                cId = postgraduateDiplomaBasicInfo.ID;
            }
            ...
            try
            {
                if (cId > 0)
                {
                    ...
                    DAL.PostgraduateDiplomaBasicInfo obj = new DAL.PostgraduateDiplomaBasicInfo();
                    using (var db = new CandidateDataManager())
                    {
                        obj = db.AdmissionDB.PostgraduateDiplomaBasicInfoes.Where(x => x.ID == cId).FirstOrDefault();
                    ...
                    if (obj != null && obj.ID > 0)  //update if object exists
                    {
                        ...
                        {
                            updateDb.Update<DAL.PostgraduateDiplomaBasicInfo>(candidate);
                        ...
                        dLog.PageName = "PostgraduateDiplomaApplicationBasic.aspx; " + _pageUrl;
                ...
                Response.Redirect("PostgraduateDiplomaApplicationEducation.aspx", false);
