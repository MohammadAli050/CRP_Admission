...
{
    public partial class ApplicationPriority : PageBase
    {
        long uId = -1;
                    ...
                    if(candidateId > 0)
                    {
                        using(var db = new CandidateDataManager())
                        {
                            List<DAL.SPAdmissionUnitProgramsByCandidateId_Result> programsAppliedList
                                    = db.AdmissionDB.SPAdmissionUnitProgramsByCandidateId(candidateId, true).ToList();
        ...
        private void LoadListView()
        {
            #region
            /*
            List<DAL.CandidateFormSl> cFormSlList;
            List<DAL.AdmissionSetup> admSetupList;
            List<DAL.AdmissionUnitProgram> admUnitProgList;
            List<DAL.AdmissionUnitProgram> tempAdmUnitProgList;
            if (uId > 0)
            {
                using (var db = new CandidateDataManager())
                {
                ...
                if (cId > 0)
                {
                    ...
                    cFormSlList = new List<DAL.CandidateFormSl>();
                    using (var db = new CandidateDataManager())
                    {
                        cFormSlList = db.GetAllCandidateFormSlByCandID_ND(cId);
                    }
                    admSetupList = new List<DAL.AdmissionSetup>();
                    if (cFormSlList.Any())
                    {
                        foreach (var item in cFormSlList)
                        {
                            DAL.AdmissionSetup tempAdmSetup = new DAL.AdmissionSetup();
                            using (var db = new CandidateDataManager())
                            {
                                tempAdmSetup = db.AdmissionDB.AdmissionSetups
                                    .Where(c => c.ID == item.AdmissionSetupID)
                                    .FirstOrDefault();
                                admSetupList.Add(tempAdmSetup);
                            }
                        }
                    }
                    admUnitProgList = new List<DAL.AdmissionUnitProgram>();
                    if (admSetupList.Any())
                    {
                        foreach (var item in admSetupList)
                        {
                            if (item != null)
                            {
                                tempAdmUnitProgList = new List<DAL.AdmissionUnitProgram>();
                                using (var db = new CandidateDataManager())
                                {
                                    tempAdmUnitProgList = db.AdmissionDB.AdmissionUnitPrograms
                                        .Where(c => c.AdmissionUnitID == item.AdmissionUnitID && c.EducationCategoryID == item.EducationCategoryID && c.IsActive == true)
                                        .ToList();
                                    admUnitProgList.AddRange(tempAdmUnitProgList);
                                }
                            }
                        }
                    }
                    ...
                    {
                        gvProgramPriority.DataSource = admUnitProgList.ToList();
                        noOfPrograms = admUnitProgList.Count();
                    }
                    else
                    {
            ...
            if(cId > 0)
            {
                try
                {
                    using (var db = new CandidateDataManager())
                    {
                        ...
                        //get only paid and undergraduate.
