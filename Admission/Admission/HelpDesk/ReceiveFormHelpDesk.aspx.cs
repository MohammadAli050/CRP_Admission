...
{
    public partial class ReceiveFormHelpDesk : PageBase
    {
        ...
        private void LoadListView(string searchText, int? acaCalId)
        {
            ...
            if (searchText != null)//search using search text
            {
                ...
                if (list.Count > 0)
                {
                    ...
                    {
                        list = list.Where(x => x.PaymentId.ToString() == searchText.Trim() || x.Mobile == searchText.Trim() || x.Email == searchText.Trim()).ToList();
                    }
        ...
        protected void lvFormRequest_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            //        lblPaid.Font.Size = FontUnit.Large;
        ...
        protected void lvFormRequest_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            {
                //Response.Redirect("~/Admission/HelpDesk/HD_ApplicationBasic.aspx?val=" + e.CommandArgument.ToString(), false);

                long candidateId = Int64.Parse(e.CommandArgument.ToString());
                string cipherCandidateId = candidateId.ToString(); 
                string url = "HD_ApplicationBasic.aspx?val=" + cipherCandidateId;
                Page.ClientScript.RegisterStartupScript(this.GetType(), "OpenWindow", "window.open('" + url + "','_blank');", true); //_newtab

            }
            #region BkashInfo N/A
            //else if (e.CommandName == "SendBkashInfo")
            //{
            //    long referenceNo = -1;
            ...
            //            eLog.Page = "ReceiveFormHelpDesk.aspx";
